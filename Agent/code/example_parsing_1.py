...
OPENROUTER_API_KEY: str = os.getenv(
    "OPENROUTER_API_KEY",
    "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",
)
OPENROUTER_API_URL = "https://openrouter.ai/api/v1/chat/completions"

OCR_MODEL  = "meta-llama/llama-4-scout-17b-16e-instruct"   # узел 1: vision OCR (Groq)
JSON_MODEL = "nvidia/nemotron-3-super-120b-a12b:free"      # узел 2: JSON-формирователь (OpenRouter)
...
def _call_groq(
    ...
    **extra_kwargs: Any,
    ...
    """Не-стриминговый вызов Groq (нужен для response_format=json_object)."""
    client = Groq(api_key=GROQ_API_KEY)
    completion = client.chat.completions.create(
        model=model,
        messages=messages,
...
def _stream_openrouter(
    ...
    """Стриминговый вызов OpenRouter (SSE), аналог OpenRouter SDK для Python."""
    import requests as _requests

    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json",
        "HTTP-Referer": "https://github.com/document-recognition",
        "X-OpenRouter-Title": "DocumentParser",
    }
    payload = {
        "model": model,
        "messages": messages,
...
def _shrink_ocr_text_for_json_node(ocr_text: str, max_chars: int = 6000) -> str:
    """
    Сокращает OCR-текст для 2-го узла:
    - сохраняет markdown-таблицы целиком (самый важный блок для items)
    - сохраняет ключевые строки с итогами/НДС/номером/датой/поставщиком/покупателем
    - ограничивает общий объём, чтобы не ловить 413/TPM limit
    """
    ...
    compact = []
    if table_lines:
        compact.append("### OCR_TABLE")
        compact.extend(table_lines)
    if key_lines:
        compact.append("\n### OCR_KEY_LINES")
        compact.extend(key_lines)

    result = "\n".join(compact).strip() or ocr_text[:max_chars]
...
def json_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        ocr_text_for_json = _shrink_ocr_text_for_json_node(state["ocr_text"], max_chars=6000)
        ...
        messages = [
...

            {
                "role": "user",
                "content": (
                    "Вот распознанный текст документа (сжатый для парсинга, таблица сохранена):\n\n"
                    f"{ocr_text_for_json}\n\n"
        ...
        # Узел 2 использует OpenRouter (google/gemma-3-12b-it:free) через SSE stream.
        raw_json_text = _stream_openrouter(
            model=JSON_MODEL,
            messages=messages,
            temperature=0,
...
def validate_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        print("✅ [Валидация] JSON прошел все проверки.")
        return {**state, "parsed_json": parsed, "validation_error": None, "error": None}
    except Exception as e:
        print(f"⚠️ [Валидация] Ошибка парсинга/валидации JSON: {e}")
...
def _route_after_validate(state: AgentState) -> str:
    if state.get("error"):
        print(f"🛑 [Маршрутизация] Остановка графа из-за ошибки узла: {state.get('error')}")
        return "done"
    if state.get("parsed_json") is not None and not state.get("validation_error"):
        print("✅ [Маршрутизация] Валидация успешна, завершаем граф.")
        return "done"
    if int(state.get("attempts", 0)) >= MAX_JSON_FIX_ATTEMPTS:
        print(
            "🛑 [Маршрутизация] Достигнут лимит ретраев "
            f"({state.get('attempts', 0)}/{MAX_JSON_FIX_ATTEMPTS}). "
            "Останавливаем граф."
        )
        return "done"  # выходим, чтобы не зациклиться; ошибка останется в validation_error
    next_attempt = int(state.get("attempts", 0)) + 1
    print(
        "🔁 [Маршрутизация] JSON не прошел валидацию, отправляем на переработку в json_node. "
        f"Следующая попытка: {next_attempt}/{MAX_JSON_FIX_ATTEMPTS}. "
        f"Причина: {state.get('validation_error')}"
    )
    return "retry"
